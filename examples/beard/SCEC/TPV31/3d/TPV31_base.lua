...
-- material properties
material = {
  D     = {0.00, 2.40, 5.00, 10.00, 15.00,},
  cp_m  = {0.00, 4.05, 5.20,  5.75,  6.50,},
  cs_m  = {0.00, 2.25, 3.05,  3.45,  3.80,},
  rho_m = {0.00, 2.58, 2.62,  2.72,  3.00,},
  cp_p  = {4.05, 4.45, 5.75,  6.50,  6.50,},
  cs_p  = {2.25, 2.55, 3.45,  3.80,  3.80,},
  rho_p = {2.58, 2.60, 2.72,  3.00,  3.00,},
}
...
function cp(x,y,z,t,xc,yc,zc)
  ...
  while material.D[k] do
    if material.D[k] > yc then
      dy = (material.D[k-1]-y)/(material.D[k-1]-material.D[k])
      V  = material.cp_p[k-1] - (material.cp_p[k-1]-material.cp_m[k])*dy
      break
    end
    V = material.cp_m[k]
    k = k + 1
...
function cs(x,y,z,t,xc,yc,zc)
  ...
  while material.D[k] do
    if material.D[k] > yc then
      dy = (material.D[k-1]-y)/(material.D[k-1]-material.D[k])
      V  = material.cs_p[k-1] - (material.cs_p[k-1]-material.cs_m[k])*dy
      break
    end
    V = material.cs_m[k]
    k = k + 1
...
function rho(x,y,z,t,xc,yc,zc)
  ...
  while material.D[k] do
    if material.D[k] > yc then
      dy = (material.D[k-1]-y)/(material.D[k-1]-material.D[k])
      V  = material.rho_p[k-1] - (material.rho_p[k-1]-material.rho_m[k])*dy
      break
    end
    V = material.rho_m[k]
    k = k + 1
