...
if cnt > 0
  ...
  fprintf(oid,'t  h-slip  h-slip-rate  h-shear-stress  v-slip  v-slip-rate v-shear-stress  n-stress\n');
  for k = 2:length(t)
    fprintf(oid,'%+30.17E %+30.17E %+30.17E %+30.17E %+30.17E %+30.17E %+30.17E %+30.17E\n',...
             t(k),hslip(k),hsliprate(k),hshearstress(k),vslip(k),vsliprate(k),vshearstress(k),nstress(k));
  end
  fclose(oid);
end
