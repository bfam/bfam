 ...
 * }
 */
static int lua_global_function_call(lua_State *L, const char *name,
                                    const char *sig, ...)
{
  va_list vl;
  int num_arg = 0;
...
static void init_mpi(blade_t *blade, MPI_Comm mpicomm)
{
...
static prefs_t *new_prefs(const char *prefs_filename)
{
  ...
  BFAM_ABORT_IF_NOT(prefs->dimension == 2 || prefs->dimension == 3,
                    "blade not set up to handle dimension %d",
                    prefs->dimension);
  ...
  prefs->brick_args->periodic_x =
      lua_get_table_int(prefs->L, "brick", "periodic_x", 1);
  prefs->brick_args->periodic_y =
      lua_get_table_int(prefs->L, "brick", "periodic_y", 1);
...
static void get_element_order(p4est_iter_volume_info_t *info, void *arg)
{
  ...
  int N = 0;
  int result = lua_global_function_call(data->L, "element_order", "ddd"
                                                                  "ddd"
                                                                  "ddd"
                                                                  "ddd"
                                                                  "ii"
                                                                  ">"
                                                                  "i",
                                        vxyz[0], vxyz[1], vxyz[2], vxyz[3],
                                        vxyz[4], vxyz[5], vxyz[6], vxyz[7],
                                        vxyz[8], vxyz[9], vxyz[10], vxyz[11],
  ...
  int result = lua_global_function_call(
      ...
      vxyz[8], vxyz[9], vxyz[10], vxyz[11], vxyz[12], vxyz[13], vxyz[14],
      vxyz[15], vxyz[16], vxyz[17], vxyz[18], vxyz[19], vxyz[20], vxyz[21],
      vxyz[22], vxyz[23], info->quad->level, info->treeid, &N);
#else
#error "Bad Dimension"
#endif
 ...
 */
static void split_domain(blade_t *blade, prefs_t *prefs)
{
...
#if DIM == 2
      lua_global_function_call(L, name, "rrrr>r", x[n], y[n], tmpz, t,
                               &field[n]);
#elif DIM == 3
      lua_global_function_call(L, name, "rrrr>r", x[n], y[n], z[n], t,
                               &field[n]);
#else
#error "Bad Dimension"
#endif
    ...
    BFAM_WARNING("Did not find '%s' in lua as a function or number using 0: "
                 "lua message: '%s'",
                 name, lua_tostring(L, -1));
  lua_pop(L, 1);

  BFAM_ROOT_INFO("field_set_val: using '%s' with value %" BFAM_REAL_FMTe, name,
                 val);
...
static void domain_add_fields(blade_t *blade, prefs_t *prefs)
{
...
#error "Bad Dimension"
  ...
  // JK     bfam_domain_add_field(domain, BFAM_DOMAIN_OR, boundary,
  // boundary_fields[g]);
  // JK
  // JK   bfam_domain_init_field(domain, BFAM_DOMAIN_OR, boundary, "_grid_x1",
  // 0,
  ...
  // JK
  // JK         bfam_domain_add_tag((bfam_domain_t*)blade->domain,
  // BFAM_DOMAIN_OR,
  // JK             this_glue, tag);
  // JK       }
  // JK       else
  ...
  // JK                 " glue %d does not contain `%s', using default
  // %"BFAM_REAL_PRIe,
  ...
  // JK           lua_pop(L, 1);
  // JK
  // JK           bfam_domain_add_field(domain, BFAM_DOMAIN_OR, this_glue,
  // sw_fields[f]);
  // JK           bfam_domain_init_field(domain, BFAM_DOMAIN_OR, this_glue,
  // sw_fields[f],
  ...
  // JK    */
  // JK   const char *default_boundary_glue[] = {"_glue_boundary","_glue_id_-1",
  // NULL};
 ...
 */
static void init_domain(blade_t *blade, prefs_t *prefs)
{
...
static void stop_blade(blade_t *blade, prefs_t *prefs)
{
                                 ...
                                 bfam_real_t *restrict JI)
{
  ...
  bfam_real_t *restrict ux = bfam_dictionary_get_value_ptr(&s->fields, "ux");
  BFAM_ASSUME_ALIGNED(ux, 32);

  bfam_real_t *restrict uy = bfam_dictionary_get_value_ptr(&s->fields, "uy");
  BFAM_ASSUME_ALIGNED(uy, 32);

#if DIM == 3
  bfam_real_t *restrict uz = bfam_dictionary_get_value_ptr(&s->fields, "uz");
...
#elif DIM == 3
    bfam_real_t u =
        BFAM_REAL_SQRT(ux[n] * ux[n] + uy[n] * uy[n] + uz[n] * uz[n]);
    bfam_real_t hr2 =
...
#define X(order)                                                               \
  case order:                                                                  \
    blade_dgx_scale_rates_advection_2_##order(sub->N, sub, rate_prefix, a);    \
    break;
#elif DIM == 3
#define X(order)                                                               \
  case order:                                                                  \
    blade_dgx_scale_rates_advection_3_##order(sub->N, sub, rate_prefix, a);    \
    break;
#else
#error "Bad Dimension"
#endif
...
static void intra_rhs_advection(int N, bfam_subdomain_dgx_t *sub,
...
#define X(order)                                                               \
  case order:                                                                  \
    blade_dgx_intra_rhs_advection_2_##order(N, sub, rate_prefix, field_prefix, \
                                            t);                                \
    break;
#elif DIM == 3
#define X(order)                                                               \
  case order:                                                                  \
    blade_dgx_intra_rhs_advection_3_##order(N, sub, rate_prefix, field_prefix, \
                                            t);                                \
    break;
#else
#error "Bad Dimension"
...
#define X(order)                                                               \
  case order:                                                                  \
    blade_dgx_add_rates_advection_2_##order(sub->N, sub, field_prefix_lhs,     \
                                            field_prefix_rhs, rate_prefix, a); \
    break;
#elif DIM == 3
#define X(order)                                                               \
  case order:                                                                  \
    blade_dgx_add_rates_advection_3_##order(sub->N, sub, field_prefix_lhs,     \
                                            field_prefix_rhs, rate_prefix, a); \
    break;
#else
#error "bad dimension"
               ...
               const bfam_long_real_t a, void *user_data)
{
  ...
  {
    bfam_subdomain_dgx_t *sub = (bfam_subdomain_dgx_t *)thisSubdomain;
    add_rates_advection(sub, field_prefix_lhs, field_prefix_rhs, rate_prefix,
                        a);
  }
  else if (bfam_subdomain_has_tag(thisSubdomain, "_glue_boundary") ||
           bfam_subdomain_has_tag(thisSubdomain, "_glue_parallel") ||
...
static void init_time_stepper(blade_t *blade, prefs_t *prefs)
{
...
static bfam_real_t compute_domain_dt(blade_t *blade, prefs_t *prefs,
                                     const char *volume[], const char *glue[],
                                     int *nsteps_ptr, int *ndisp_ptr,
                                     int *noutput_ptr, int *nerr_ptr)
{
  ...
  BFAM_MPI_CHECK(MPI_Allreduce(&min_ldt, &min_global_dt, 1, BFAM_REAL_MPI,
                               MPI_MIN, blade->mpicomm));

  bfam_real_t dt = 0;
  int result = lua_global_function_call(prefs->L, "time_step_parameters",
                                        "r>riii", min_global_dt, &dt,
                                        nsteps_ptr, ndisp_ptr, noutput_ptr);
  ...
  init_time_stepper(blade, prefs);

  return dt;
}

static bfam_real_t compute_energy(blade_t *blade, prefs_t *prefs, bfam_real_t t,
                                  const char *prefix)
{
...
#define X(order)                                                               \
  case order:                                                                  \
    blade_dgx_energy_2_##order(sub->N, &energy_local, sub, prefix);            \
    break;
#elif DIM == 3
#define X(order)                                                               \
  case order:                                                                  \
    blade_dgx_energy_3_##order(sub->N, &energy_local, sub, prefix);            \
    break;
#else
#error "Bad Dimension"
#endif
                        ...
                        void *arg, bfam_real_t *restrict err)
{
  ...
  {
#if DIM == 2
    lua_global_function_call(L, name + 6, "rrrr>r", x[n], y[n], tmpz, t,
                             &err[n]);
#elif DIM == 3
    lua_global_function_call(L, name + 6, "rrrr>r", x[n], y[n], z[n], t,
                             &err[n]);
#else
#error "Bad Dimension"
#endif
...
static void run_simulation(blade_t *blade, prefs_t *prefs)
{
  const char *volume[] = {"_volume", NULL};
  const char *glue[] = {"_glue_parallel", "_glue_local", NULL};
  ...
  bfam_real_t dt = compute_domain_dt(blade, prefs, volume, glue, &nsteps,
                                     &ndisp, &noutput, &nerr);
  ...
  if (noutput >= 0)
  {
    ...
    snprintf(output, BFAM_BUFSIZ, "%s/%s_pxest_mesh", prefs->data_directory,
             prefs->output_prefix);
    p4est_vtk_write_all(blade->domain->pxest, NULL, 1, 1, 1, 1, 0, 0, 0,
                        output);
    ...
    bfam_vtk_write_file((bfam_domain_t *)blade->domain, BFAM_DOMAIN_OR, volume,
                        prefs->data_directory, output, (0) * dt, fields, NULL,
  ...
  for (int s = 1; s <= nsteps; s++)
  {
    ...
    if (s % ndisp == 0)
    {
      ...
      {
        BFAM_ROOT_INFO("\x1B[%dm"
                       "time: %10.5" BFAM_REAL_PRIe "\x1B[0m",
                       34, s * dt);
      }
      else
      {
        ...
        BFAM_ROOT_INFO(
            ...
            "\x1B[0m",
            color, s * dt, initial_energy, new_energy,
    ...
    if (noutput > 0 && s % noutput == 0)
    {
      ...
      bfam_vtk_write_file((bfam_domain_t *)blade->domain, BFAM_DOMAIN_OR,
                          volume, prefs->data_directory, output, (s)*dt, fields,
    ...
    if (nerr > 0 && s % nerr == 0)
    {
      ...
      for (int f = 0; err_flds[f] != NULL; f++)
        bfam_domain_init_field((bfam_domain_t *)blade->domain, BFAM_DOMAIN_OR,
                               volume, err_flds[f], s * dt, check_error,
                               &err_args);
      ...
      if (noutput > 0)
      {
        ...
        snprintf(err_output, BFAM_BUFSIZ, "%s_error_%05d", prefs->output_prefix,
                 s);
        bfam_vtk_write_file((bfam_domain_t *)blade->domain, BFAM_DOMAIN_OR,
                            volume, prefs->data_directory, err_output, (s)*dt,
                            err_flds, NULL, NULL, prefs->vtk_binary,
                            prefs->vtk_compress, 0);
        energy = new_energy;
      }
    }
  }
