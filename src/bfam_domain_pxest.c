...
static void interpolate_data_volume(const bfam_real_t *src, int8_t c_src,
                                    int N_src, bfam_real_t *dst, int8_t c_dst,
...
void bfam_domain_pxest_free(bfam_domain_pxest_t *domain)
{
  ...
  if (domain->N2N)
  {
    bfam_dictionary_allprefixed_ptr(domain->N2N, "", clear_N2N_dict, NULL);
                                                  ...
                                                  void *arg)
{
  ...
  for (bfam_locidx_t k = 0; k < fd->subdomain_dst->K; ++k)
  {
    ...
    for (int c = 0; c < num_src; ++c)
    {
      ...
      interpolate_data_volume(src_data, chld_id_src[c], subdomain_src[c]->N,
                              dst_data, fd->maps->dst_to_dst_chld_id[q_id_dst],
                              fd->subdomain_dst->N, flags, fd->N2N);
