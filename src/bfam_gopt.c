        ...
        if( '-' == (*arg_p)[1] ) {
          ...
          for( ; opt_spec_p-> key; ++opt_spec_p ) {
            if( strchr( opt_spec_p-> shorts, (*arg_p)[1] )){
              opt_count+= opt_spec_p-> flags & BFAM_GOPT_ARG ? 1 : strlen( (*arg_p) + 1 );
              break;
            }
          }
        }
          ...
          {{{
            ...
            while( *longs ){
              ...
              if( '=' == *option_cp || !*option_cp ){
                if( *name_cp ){
                  if( next_option-> key ){
                    BFAM_LERROR("%s: --%.*s: abbreviated option is ambiguous\n", argv[0], (int)( option_cp -( (*arg_p) + 2 )), (*arg_p) + 2 );
                    bfam_free( opts );
                    exit( EX_USAGE );
                  }
            ...
            if( !( opt_spec_p-> flags & BFAM_GOPT_REPEAT )){
              ...
              for( ; opt_p != next_option; ++opt_p )
                if( opt_p-> key == opt_spec_p-> key ){
                  BFAM_LERROR("%s: --%.*s: option may not be repeated (in any long or short form)\n", argv[0], (int)strcspn( (*arg_p) + 2, "=" ), (*arg_p) + 2 );
                  bfam_free( opts );
                  exit( EX_USAGE );
                }
            }
            if( opt_spec_p-> flags & BFAM_GOPT_ARG ){
              ...
              if( (char*)0 + 1 == next_option-> arg ){
                ...
                if( !*arg_p || ('-' == (*arg_p)[0] && (*arg_p)[1]) ){
                  BFAM_LERROR("%s: --%s: option requires an option argument\n", argv[0], (*(arg_p-1)) + 2 );
                  bfam_free( opts );
                  exit( EX_USAGE );
                }
              ...
              if( strchr( (*arg_p) + 2, '=' )){
                BFAM_LERROR("%s: --%.*s: option may not take an option argument\n", argv[0], (int)strcspn( (*arg_p) + 2, "=" ), (*arg_p) + 2 );
                bfam_free( opts );
                exit( EX_USAGE );
              }
            ...
            for( ; opt_spec_p-> key; ++opt_spec_p )
              if( strchr( opt_spec_p-> shorts, *short_opt )){
                if( !( opt_spec_p-> flags & BFAM_GOPT_REPEAT )){
                  ...
                  for( ; opt_p != next_option; ++opt_p )
                    if( opt_p-> key == opt_spec_p-> key ){
                      BFAM_LERROR("%s: -%c: option may not be repeated (in any long or short form)\n", argv[0], *short_opt );
                      bfam_free( opts );
                      exit( EX_USAGE );
                    }
                }
                ...
                if( opt_spec_p-> flags & BFAM_GOPT_ARG ){
                  if( short_opt[1] )
                    ...
                    ++arg_p;
                    if( !*arg_p || ('-' == (*arg_p)[0] && (*arg_p)[1]) ){
                      BFAM_LERROR("%s: -%c: option requires an option argument\n", argv[0], *short_opt );
                      bfam_free( opts );
                      exit( EX_USAGE );
                    }
                    next_option-> arg= *arg_p;
...
size_t bfam_gopt_args( const void *vptr_opts, int key, const char **args, size_t args_len ){
...
void bfam_gopt_free( void *vptr_opts ){
  bfam_free( vptr_opts );
}
