...
static int
bfam_subdomain_dgx_quad_glue_get_face_scalar_fields_m(const char * key,
    void *val, void *arg)
{
  bfam_subdomain_dgx_quad_get_put_data_t *data =
    (bfam_subdomain_dgx_quad_get_put_data_t*) arg;
  ...
  const bfam_real_t *restrict sub_m_face_field =
    bfam_dictionary_get_value_ptr(&sub->sub_m->base.fields_face, key);
  ...
  for(bfam_locidx_t k = 0; k < K; ++k)
  {
    ...
    const bfam_real_t *restrict sub_m_face_elem =
      sub_m_face_field + sub_m_Nfp*(face + 4*EToEm[k]);
                                  ...
                                  bfam_subdomain_face_map_entry_t *mapping)
{
  ...
  for(int i = 0; i < subdomain->num_interp; ++i)
  {
    ...
    subdomain->projection[i] =
        bfam_malloc_aligned(Nrp * sub_m_Nrp * sizeof(bfam_real_t));

    for(int n = 0; n < Nrp*sub_m_Nrp; ++n)
      subdomain->projection[i][n] = (bfam_real_t) projection[i][n];
  }
...
bfam_subdomain_dgx_quad_glue_free(bfam_subdomain_t *subdomain)
{
  ...
  for(int i = 0; i < sub->num_interp; ++i)
    if(sub->projection[i])
      bfam_free_aligned(sub->projection[i]);
  bfam_free_aligned(sub->projection);
