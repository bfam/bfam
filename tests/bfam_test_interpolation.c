...
bilinear_test()
{
  ...
  bfam_util_linear_blend(x,NULL,NULL,N,NULL,NULL,xc,yc,zc);
  BFAM_ABORT_IF_NOT(fabs(x[N[0]/2]) < 1e-15,"1d interp fail");

  bfam_util_linear_blend(x,y,NULL,N,NULL,NULL,xc,yc,zc);
  BFAM_ABORT_IF_NOT(fabs(x[N[0]/2+N[1]/2*(N[0]+1)]) < 1e-15 &&
      fabs(y[N[0]/2+N[1]/2*(N[0]+1)]) < 1e-15, "2-D interp fail");

  bfam_util_linear_blend(x,y,z,N,NULL,NULL,xc,yc,zc);
  BFAM_ABORT_IF_NOT(
